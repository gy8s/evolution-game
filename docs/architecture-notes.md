...
Twenty-two source files have been extracted and are inlined back into `game/play.html` by `scripts/build_play_html.mjs`. The playable file still ships all content inline, so it works with no build step at runtime. The build script replaces only the regions between these marker comments:
...
| 5514–5741 | Debug/QA function declarations (addDebugTrace, debugRoll, importantStateSnapshot, etc.) |
...
| 5789–5881 | Encounter helper functions (getEncounterTemplate, validateEncounterData, normaliseEncounter) |
