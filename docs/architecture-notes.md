...
Twenty-three source files have been extracted and are inlined back into `game/play.html` by `scripts/build_play_html.mjs`. The playable file still ships all content inline, so it works with no build step at runtime. The build script replaces only the regions between these marker comments:
...
```
The debug/QA helper functions sit between the two regions and are themselves inlined via their own generated region (see `src/qa/debug-helpers.js` below). The encounter helper functions (`getEncounterTemplate`, `validateEncounterData`, `normaliseEncounter`) follow region [2/2] and also remain in `game/play.html`. Game state globals should be edited in `src/state/game-state-globals.js`, not inside the generated regions of `game/play.html`.
