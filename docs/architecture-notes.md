...
Twenty-one source files have been extracted and are inlined back into `game/play.html` by `scripts/build_play_html.mjs`. The playable file still ships all content inline, so it works with no build step at runtime. The build script replaces only the regions between these marker comments:
...
```
This region sits immediately after the `// ===== GAME INIT WITH PROFILE =====` banner, which stays outside the generated region. Only `initGame` is extracted; the global constant and state declarations that follow (starting with `const KNOWLEDGE_MAX`) are not part of this region and remain in `game/play.html`. Game bootstrap should be edited in `src/bootstrap/game-init.js`, not inside the generated region of `game/play.html`.
...
| 5470–5852 | Game state initialisation: global constants and state declarations |
