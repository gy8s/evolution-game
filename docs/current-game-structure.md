...
Last updated: 2026-06-09 (lineage progression framework added — player starts as Purgatorius and can evolve along a concrete taxon route Purgatorius → Teilhardina → Notharctus → Eosimias by reaching the adult life stage and mating 3 times; new `src/state/lineage.js` module; lineage panel UI; death-screen evolve option; profile-level persistence of current stage and completed stages). Earlier: Phase 1 complete — 26 source extractions; build scaffold and architecture flowcharts in README.md; creature layers+subtypes modularity implemented.
...
Twenty-five source extractions are complete. The playable file `game/play.html` still contains all content inline (between marker comments) so it works with no build step or runtime dependency.

| Source file | What it contains | In play.html |
|-------------|-----------------|--------------|
| `src/styles/game.css` | All CSS | Inline `<style>` block |
| `src/data/encounter-data.js` | `encounters` (each entry now embeds `layers` and `subtypes`); `hiddenSubtypePools` removed | Two inline JS regions ([2/2] is now empty) |
...
| `src/state/lineage.js` | Lineage progression data + helpers (`lineageStages`, `getLineageStage`, `getInitialLineageState`, `profileLoadLineage`, `profileSaveLineage`, `lineageRecordAdult`, `lineageRecordMating`, `checkLineageUnlock`, `lineageEvolveNow`, `renderLineagePanel`) | One inline JS region (~line 5324) |
...
| `src/engine/encounter-table-builder.js` | `buildEncounterTables()` — generates `encounterTables` from per-creature `layers` data after `encounters` is defined | One inline JS region (~line 5915); followed by `const encounterTables = buildEncounterTables();` call |
...
The player follows a concrete taxon route on the human-line: **Purgatorius → Teilhardina → Notharctus → Eosimias**. These are presented as representative lineage stages, not a guaranteed direct ancestor-descendant chain.

Each stage unlocks the next when two requirements are met in the current run:

1. The player reaches the adult life stage (the existing `isPlayerMature()` growth gate, `player.size >= 5.3`).
2. The player mates successfully at least **3** times (counted from the existing successful-mating logic).

State lives on `player.lineage`:

- `currentStageId`, `completedStages` — persist at the **profile** level (via `profileLoadLineage` / `profileSaveLineage`, mirroring the Field Journal pattern). A restart resumes as the current species; death never erases unlocked progression.
- `stageProgress.adultReached`, `stageProgress.successfulMatingCount`, `evolutionUnlocked` — per-run; reset when a fresh run starts. Resume restores them with the rest of the `player` snapshot.

Hooks into existing gameplay: `lineageRecordAdult()` is called from the growth/maturity tick; `lineageRecordMating()` is called from both successful-mating paths; `checkLineageUnlock()` evaluates the two gates and marks `evolutionUnlocked`.

Once unlocked, evolution is never forced. The lineage panel (`renderLineagePanel`, in the Player panel) shows current species, epoch, adult status, matings `x / 3`, and an **Evolve now** / **Continue exploring** choice. If the player dies with evolution unlocked, the death screen offers **Evolve to [next species]** alongside **Restart as [current species]** and **Stay on this run**. `lineageEvolveNow()` records the finished run, advances and persists the stage, then starts a fresh run as the next species.

Eosimias is the end of the currently implemented arc (`nextStageId: null`): completing it shows "Current lineage arc complete" and allows continued exploration without pointing at missing next-stage data. The next stage (likely Aegyptopithecus) is left as a TODO for the future ecosystem/encounter-table work. The internal `player.species` label and encounter narration are intentionally unchanged by this framework.
...
| ~5324–5556 | Lineage progression data + helpers — generated, source is `src/state/lineage.js` (line ranges below this point shift down accordingly) |
...
| 5792–5983 | Encounter helper functions — generated, source is `src/engine/encounter-helpers.js`: getEncounterTemplate, validateEncounterData, normaliseEncounter |
| 5986–6045 | Encounter table builder — generated, source is `src/engine/encounter-table-builder.js`: buildEncounterTables |
| 6048–6131 | Core utilities — generated, source is `src/utils/core-utils.js`: pure helpers (clamp, roll, choice, clonePlain, escapeHtml, etc.) |
| 6132–6265 | Remaining utilities: logging, debug helpers, narration setters |
| 6267–6269 | Encounter data [2/2] — empty region, source is `src/data/encounter-data.js` [2/2] |
...
2. Static encounter data and spawn tables — **`encounters` (with embedded `layers`+`subtypes`) extracted to `src/data/encounter-data.js`; `buildEncounterTables()` extracted to `src/engine/encounter-table-builder.js`; `encounterTables` and `hiddenSubtypePools` removed** ✓
...
All creature definitions in `src/data/encounter-data.js` now carry their own `layers` and `subtypes` data as described in these CSVs. The separate `encounterTables` and `hiddenSubtypePools` globals have been removed.
