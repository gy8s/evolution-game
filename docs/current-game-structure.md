...
Last updated: 2026-06-06 (Phase 1 complete — 25 source extractions done; build scaffold and architecture flowcharts added to README.md; Phase 2 candidates listed in docs/project-plan.md; creature schema CSV docs added; layers+subtypes modularity implemented — `encounters` now embeds `layers` and `subtypes` per creature; `encounterTables` generated by `buildEncounterTables()`; `hiddenSubtypePools` removed).
